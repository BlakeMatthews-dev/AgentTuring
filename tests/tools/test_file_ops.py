...
async def test_read_file(executor, workspace):
    result = await executor.execute({"action": "read", "path": "hello.txt", "workspace": str(workspace)})
    assert result.success is True
    assert result.content == "world"


async def test_read_file_not_found(executor, workspace):
    result = await executor.execute({"action": "read", "path": "nope.txt", "workspace": str(workspace)})
...
async def test_list_not_a_directory(executor, workspace):
    result = await executor.execute({"action": "list", "path": "hello.txt", "workspace": str(workspace)})
    assert result.success is False
    assert "not a directory" in result.error


async def test_mkdir(executor, workspace):
    result = await executor.execute({"action": "mkdir", "path": "new_dir/sub", "workspace": str(workspace)})
    assert result.success is True
    assert (workspace / "new_dir" / "sub").is_dir()


async def test_exists_true(executor, workspace):
    result = await executor.execute({"action": "exists", "path": "hello.txt", "workspace": str(workspace)})
    assert result.success is True
    data = json.loads(result.content)
...
async def test_exists_false(executor, workspace):
    result = await executor.execute({"action": "exists", "path": "nope.txt", "workspace": str(workspace)})
    assert result.success is True
    data = json.loads(result.content)
...
async def test_workspace_not_found(executor):
    result = await executor.execute({"action": "read", "path": "x", "workspace": "/nonexistent/path"})
    assert result.success is False
    assert "not found" in result.error
...
async def test_path_escape_blocked(executor, workspace):
    ...
    result = await executor.execute({"action": "read", "path": "../../etc/passwd", "workspace": str(workspace)})
    assert result.success is False
