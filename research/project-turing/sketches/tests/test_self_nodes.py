...
def test_ac_24_1_passion_default_rank_and_strength(srepo, bootstrapped_id, new_id) -> None:
    p = note_passion(srepo, bootstrapped_id, "work that lasts", 0.8, new_id)
    assert p.strength == 0.8
    assert p.rank == 0
    assert p.self_id == bootstrapped_id
    p2 = note_passion(srepo, bootstrapped_id, "craft over speed", 0.7, new_id)
    assert p2.rank == 1


def test_ac_24_1_passion_strength_oor_raises(srepo, bootstrapped_id, new_id) -> None:
    with pytest.raises(ValueError):
        note_passion(srepo, bootstrapped_id, "x", 1.1, new_id)


def test_ac_24_1_passion_duplicate_text_case_insensitive_raises(
    srepo, bootstrapped_id, new_id
) -> None:
    note_passion(srepo, bootstrapped_id, "Work that Lasts", 0.5, new_id)
...
def test_ac_24_2_hobby_last_engaged_at_starts_none(srepo, bootstrapped_id, new_id) -> None:
    h = note_hobby(srepo, bootstrapped_id, "Reading", "philosophy of mind", new_id)
    assert h.last_engaged_at is None


def test_ac_24_2_hobby_duplicate_raises(srepo, bootstrapped_id, new_id) -> None:
    note_hobby(srepo, bootstrapped_id, "Reading", "X", new_id)
    with pytest.raises(ValueError, match="duplicate hobby"):
        note_hobby(srepo, bootstrapped_id, "reading", "Y", new_id)


# --------- AC-24.3 interests ------------------------------------------------


def test_ac_24_3_interest_last_noticed_none(srepo, bootstrapped_id, new_id) -> None:
    i = note_interest(srepo, bootstrapped_id, "Neuroscience", "brain stuff", new_id)
    assert i.last_noticed_at is None


def test_ac_24_3_interest_duplicate_raises(srepo, bootstrapped_id, new_id) -> None:
    note_interest(srepo, bootstrapped_id, "Neuroscience", "", new_id)
...
def test_ac_24_4_preference_unique_kind_target(srepo, bootstrapped_id, new_id) -> None:
    note_preference(srepo, bootstrapped_id, PreferenceKind.LIKE, "cats", 0.8, "", new_id)
    with pytest.raises(ValueError, match="duplicate preference"):
        note_preference(srepo, bootstrapped_id, PreferenceKind.LIKE, "cats", 0.5, "", new_id)


def test_ac_24_4_preference_different_kinds_ok(srepo, bootstrapped_id, new_id) -> None:
    note_preference(srepo, bootstrapped_id, PreferenceKind.LIKE, "cats", 0.8, "", new_id)
    # Same target, different kind: allowed.
    note_preference(srepo, bootstrapped_id, PreferenceKind.FAVORITE, "cats", 0.9, "", new_id)
...
def test_ac_24_5_skill_duplicate_raises(srepo, bootstrapped_id, new_id) -> None:
    ...
    with pytest.raises(ValueError, match="duplicate skill"):
        note_skill(srepo, bootstrapped_id, "Python", 0.5, SkillKind.INTELLECTUAL, new_id)


# --------- AC-24.6 rerank passions ------------------------------------------


def test_ac_24_6_rerank_passions_atomic(srepo, bootstrapped_id, new_id) -> None:
    p1 = note_passion(srepo, bootstrapped_id, "A", 0.5, new_id)
    p2 = note_passion(srepo, bootstrapped_id, "B", 0.5, new_id)
    p3 = note_passion(srepo, bootstrapped_id, "C", 0.5, new_id)
    result = rerank_passions(srepo, bootstrapped_id, [p3.node_id, p1.node_id, p2.node_id])
...
def test_ac_24_6_rerank_passions_incomplete_list_raises(srepo, bootstrapped_id, new_id) -> None:
    p1 = note_passion(srepo, bootstrapped_id, "A", 0.5, new_id)
    note_passion(srepo, bootstrapped_id, "B", 0.5, new_id)
    with pytest.raises(ValueError, match="ordered_ids must match"):
        rerank_passions(srepo, bootstrapped_id, [p1.node_id])


def test_ac_24_6_rerank_passions_phantom_id_raises(srepo, bootstrapped_id, new_id) -> None:
    p1 = note_passion(srepo, bootstrapped_id, "A", 0.5, new_id)
    p2 = note_passion(srepo, bootstrapped_id, "B", 0.5, new_id)
    with pytest.raises(ValueError, match="ordered_ids must match"):
        rerank_passions(srepo, bootstrapped_id, [p1.node_id, p2.node_id, "phantom"])


# --------- AC-24.10 practice_skill updates last_practiced_at ---------------


def test_ac_24_10_practice_updates_last_practiced_at(srepo, bootstrapped_id, new_id) -> None:
    ...
    after_skill = practice_skill(srepo, bootstrapped_id, s.node_id)
    assert after_skill.last_practiced_at > before


def test_ac_24_15_practice_cannot_lower_stored_level(srepo, bootstrapped_id, new_id) -> None:
    ...
    with pytest.raises(ValueError, match="cannot lower"):
        practice_skill(srepo, bootstrapped_id, s.node_id, new_level=0.5)


def test_ac_24_15_downgrade_lowers_and_requires_reason(srepo, bootstrapped_id, new_id) -> None:
    s = note_skill(srepo, bootstrapped_id, "python", 0.8, SkillKind.INTELLECTUAL, new_id)
    s2 = downgrade_skill(srepo, bootstrapped_id, s.node_id, 0.4, "long disuse after injury")
...
def test_ac_24_14_current_level_is_pure_no_write(srepo, bootstrapped_id, new_id) -> None:
    s = note_skill(srepo, bootstrapped_id, "python", 0.8, SkillKind.INTELLECTUAL, new_id)
